...
const ResourceImgModal = ({ data }) => {

    const { open, setOpen, imgUrl, resourceId } = data
    const [image, setImage] = useState(null)
    const [url, setUrl] = useState(imgUrl)
    const [loading, setLoading] = useState(false)
    const [isLoading, setIsLoading] = useState(false);//for blocking overlay
    ...
    const handlerSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('/api/resources/image', {
                method: 'POST',
                body: formData,
            });
...
export default ResourceImgModal
